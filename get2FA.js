...
export const get2FA = (s, d = 6, t = Date.now() / 30000, g = crypto.subtle, u = Uint8Array) =>
  g
    .importKey(
      "raw",
      u.from(
        Array.from(s, (c) => (+c ? +c + 56 : c.charCodeAt(0) - 1).toString(2).slice(-5))
          .join("")
          .match(/.{8}/g),
        (b) => +("0b" + b)
      ),
    ...
    .then((k) =>
      g
        ...
        .then((s) => ((e) => (u + 1e9 + ((e.getUint32(e.getUint8(19) & 15) << 1) >>> 1)).slice(-d))(new DataView(s)))
    )
