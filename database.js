export const isSecret = (arg) => {
  ...
  if (arg === "") return false
  for (let i = 0; i < arg.length; i++) if (!"ABCDEFGHIJKLMNOPQRSTUVWXYZ234567".includes(arg[i])) return false
  return true
}
...
class Database {
  ...
  addAccount(newAccount) {
    ...
    for (const account of this.accountMap.values())
      if (account.secret === newAccount.secret) throw new ErrorAccountSecretAlreadyExists()
